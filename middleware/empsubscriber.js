...
// Check either breeder has limit finished or not.. 
// Is he can register employee more ..
// Is subscription limit reached ? 
let employeesubscriber = async (req, res, next) => {
  ...
  if (breederId) {
    ...
    if (countallowed <= countemployees.length) {
      return res.json({
        status: 400, message: "No more employees allowed (subscription limit reached)", data: {}
      });
    }
  }
  next()
