    ...
    async createdefault(req,res,next){
      ...
      if(req.user && req.user._id){
        ...
        Subscription.findOne({defaultPackage:true}).then( 
            result=> 
            {
                const subscriber = new Subscriber({breederId,fromDate:Date.now(),toDate:Date.now() + result.period*24*60*60*1000,
                                subscriptionId:result.subscriptionId,allowedEmp:result.allowedEmp,allowedAnimal:result.allowedAnimal,
                                name:result.name
                                 })

                /////transaction not delete it is for lof purpose
                const transaction = new Transaction({breederId,fromDate:Date.now(),toDate:Date.now() + result.period*24*60*60*1000,
                    subscriptionId:result.subscriptionId,allowedEmp:result.allowedEmp,allowedAnimal:result.allowedAnimal,
                  })
                transaction.save((err, doc) => {
